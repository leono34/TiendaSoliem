...
class Compras extends Controller{
    ...
    public function listar_historial()
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            if ($data[$i]['estado'] == 1) {
                ...
                $data[$i]['acciones'] = '<div>
                <a href="'.base_url. "compras/generarPdf/" . $data[$i]['id'] . '" target="_blank" class="btn btn-danger"><i class="fas fa-file-pdf"></i></a>
    ...
    public function listar_ventas()
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            if ($data[$i]['estado'] == 1) {
                $data[$i]['estado'] = '<span class="badge badge-success">Pagada</span>';
                $data[$i]['acciones'] = '<div>
                <a href="' . base_url . "compras/generarVentaPdf/" . $data[$i]['id'] . '" target="_blank" class="btn btn-danger"><i class="fas fa-file-pdf"></i></a>
    ...
    public function generarPdf($folio)
    {
        ...
        $proveedor = $this->model->getCliente('proveedor',$detalle[0]['id_proveedor']);
        ...
        $pdf->Cell(27, 5, 'Nombre', 0, 0, 'L', true);
        $pdf->Cell(25, 5, utf8_decode('Teléfono'), 0, 0, 'L', true);
        $pdf->Cell(25, 5, utf8_decode('Dirección'), 0, 1, 'L', true);

        $pdf->SetFillColor(255, 255, 255);
        $pdf->SetTextColor(0, 0, 0);
        $pdf->Cell(27, 5, utf8_decode($proveedor['nombre']), 0, 0, 'L');
        $pdf->Cell(25, 5, utf8_decode($proveedor['telefono']), 0, 0, 'L');
        $pdf->Cell(25, 5, utf8_decode($proveedor['direccion']), 0, 1, 'L');
        ...
        $pdf->Cell(12, 5, 'Total', 0, 1, 'L', true);
        ...
        foreach ($detalle as $row) {
            ...
            $pdf->Cell(45, 5, $row['descripcion'], 0, 0, 'L');
            $pdf->Cell(12, 5, $row['precio'], 0, 0, 'L');
            $pdf->Cell(12, 5, $row['sub_total'], 0, 1, 'L');
    ...
    public function generarVentaPdf($folio)
    {
        ...
        $cliente = $this->model->getCliente('clientes', $detalle[0]['id_cliente']);
        require('Libraries/fpdf/fpdf.php');
        ...
        $pdf->SetFillColor(255, 255, 255);
        $pdf->SetTextColor(0, 0, 0);
        $pdf->Cell(27, 5, utf8_decode($cliente['nombre']), 0, 0, 'L');
        $pdf->Cell(25, 5, utf8_decode($cliente['telefono']), 0, 0, 'L');
        $pdf->Cell(25, 5, utf8_decode($cliente['direccion']), 0, 1, 'L');
        ...
        foreach ($detalle as $row) {
            ...
            $pdf->Cell(14, 5, $row['precio'], 0, 0, 'L');
